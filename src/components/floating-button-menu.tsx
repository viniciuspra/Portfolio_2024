...
export default function FloatingButtonMenu() {
  ...
  return (
    <div className="fixed top-1/2 right-10 transform translate-y-2/4 translate-x-2/4">
      <button
        className="rounded-full w-10 h-10 cursor-pointer transition-all hover:brightness-110 hover:animate-pulse flex justify-center items-center"
        onClick={toggleMenu}
      >
        <Icon />
      </button>
      {isMenuOpen && (
        <div className="absolute top-1/2 transform -translate-y-1/2 right-11 bg-background border-1.5 border-card-stroke z-50 w-60 rounded-md h-fit">
